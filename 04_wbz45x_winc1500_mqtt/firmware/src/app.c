    ...
    switch ( appData.state )
    {
...
#endif
            printf("Welcome to WBZ451 - WINC1500 MQTT example\r\n");
        ...
        case APP_STATE_SERVICE_TASKS:
        {
            if (OSAL_QUEUE_Receive(&appData.appQueue, &appMsg, OSAL_WAIT_FOREVER))
            {
                ...
                else if (p_appMsg->msgId==APP_MSG_MQTT)
                {
                    ssize_t nr;
                    char readByte = 0x00;
                    nr = SYS_CONSOLE_Read( SYS_CONSOLE_DEFAULT_INSTANCE, &readByte, 1 );
                    if (nr > 0)
                    {
                        readBuffer[readCount++] = readByte;
                        if (readByte == 0x0A)
                        {
                            APP_MQTT_Uart_SendMsg(readBuffer, readCount);
                            readCount = 0;
